...
async function startVerify(){if(faceBusy)return;try{faceBusy=true;setVerifyStatus('Face model load हो रहा है...');await loadModels();verifyStream=await safeFaceCamera();const v=document.getElementById('workerVerifyVideo');v.srcObject=verifyStream;document.getElementById('workerVerifyCamera').style.display='block';document.getElementById('workerVerifyScan').disabled=false;setVerifyStatus('Camera तैयार है। अब Verify Face दबाएँ।','ok')}catch(e){setVerifyStatus('❌ Camera/Face model error: '+e.message,'err')}finally{faceBusy=false}}
